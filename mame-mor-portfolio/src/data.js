  ...
  const game_thumbnails = [
       {
      id: "Touché-Tempo",
      content: './thumbnails/ToucheTempo.png',
      name: "Touché Tempo",
      role: "Gameplay Programmer & Designer",
      engine: "Unity, C#", 
      team: "6",
      type: "Game",
      devTime: "In Development - 3 Months",
      page: "https://mankirt-hothi.itch.io/touche-tempo",
      ...
      gallery: [`/Images/TempoIntro.png`, `/Images/TempoSwan.png`, `/Images/TempoWin.png`, `/Images/TempoLose.png`, `/Images/TempoParry.png`, `/Images/TempoDifficulty.png`, `/Images/TT-Controls.png`, `/Images/TT-Outlines.png` ],
      systemLink: [],
      systemImage: ["/Images/TempoEnemy.png", "/Images/TempoFMOD.png", "/Images/TT-Tutorial.png"],
      systemVideo: ["", ""],
      systemName: ["Beatmap Creation", "Enemy Stance Mapping via FMOD Event Callbacks", "Tutorialization"],
      systemDesc: ["/~Making the Beatmap~/Initially we used FMOD destination markers to create windows where the enemy would strike and mark which attacks they would perform. While this worked, there was some audio desync and the core aspect of beatmap creation was tedious. We switched over to a dictionary system as a solution. With beat events being assigned to a dictionary, this became as simple as selecting an enemy attack using a state value, and assigning a beat interval to determine attack windows. /~Creating Beat Events~/To make beatmap creation easier than using FMOD destination markers, it needed to be accessible in the inspector in a readable way. To do this I created an object for individual beat events and mapped these to a dictionary holding their key; which attack the enemy would perform (enemy stance); and how long the enemy would hold that attack for (beat interval length). ", "/~Using FMOD Event Callbacks~/On every beat the enemy stance and beat interval is retrieved from the beatmap dictionary. A public method within the enemy script is called using these stance and interval values as parameters to perform the corresponding attack. To make sure attacks or not overwritten and performed at the correct time, the next available beat is kept track of. Because beats reset after every bar, an index value is necessary and incremented after every beat event to perform the right attacks in sync with the FMOD music.", "/~Using Events for Tutorial~/During the playtesting phase of Touché Tempo, one of our main pointers was for more tutorialization to limit learning on the fly as the game was relatively difficult. I wanted tutorials to be appear as mechanics were used, and wanted them to be easily implemented into different levels for different mechanics without altering dependencies. To do this I used events in the tutorial manager script. Certain mechanics like dodges, perfect parries, or feints would invoke these events with parameters if necessary and any scene that had a tutorial manager would be listening for this to then display necessary tutorial UI. Tutorial steps were designed as an object which were fed into a list and called accordingly. What inspired this approach was my previous use of Event Listeners in mobile development, and realizing this game flow could use the exact same thing to avoid direct, manual function calls."]
    },
    {
      id: "Upperdut",
    ...
    },
    {
      ...
      name: "Endzone", 
      role: "Gameplay Programmer | Game & Level Designer",
      engine: "GB Studio",
      team: "1",
      type: "Game",
      devTime: "2 Weeks",
    ...
    },
    {
      ...
      name: "Bus Rush",
      role: "Gameplay Programmer | Designer | 3D Artist | Level Designer",
      ...
      video: "",
      gallery: [`/Images/BusCity.png`, `/Images/City.png`, `/Images/BusRushMap.png`],
    ...
    {
      id: "TomaGames",
      content: '/thumbnails/TomaThumbnail.png',
      name: "Bus Rush",
      role: "Backend Programmer | Database Designer",
      engine: "Python, React, FastAPI, Supabase",
      team: "4",
      type: "E-Commerce Store",
      devTime: "2 Months",
      page: "https://www.tomagames.store/",
      desc: "TomaGames Store is an E-commerce site for purchasing video games. Search an endless catalog with real-time item updates and a personalized user experience. ",
      video: "",
      gallery: [``],
      systemImage: [``],
      systemVideo: [""],
...

    {
      ...
      team: "3",
      type: "Mobile Game",
      devTime: "1 Month - CURRENTLY CONVERTING BACKEND TO JAVA",
      ...
      systemDesc: ["/~Tile Grid State Management~/GameScreen is a stateful widget that controls the board, score, rounds, and animations. We rendered the grid using GridView.builder with parameters allowing dynamic board sizes. This was especially important for creating bigger tilesets in more difficult game modes without doing it manually. Tiles are abstracted into a reusable FlipTile widget with animations that the user can toggle on and off in the settings menu (for performance). \n\nThe board is generated using a seeded random number generator:\n\nfinal rng = seed != null ? Random(seed) : Random();\npairs.shuffle(rng);/~Tile Matching~/Each tile reveals a shape drawn using the CustomPainter. On tap:\n- First tile starts a score decay timer\n- Second tile checks for a match\n- If tiles match --> Score added\n- If tiles mismatch --> tiles flip to unrevealed w/ delay",
        "From a design standpoint we wanted to deviate from the typical singleplayer aspect of a memory matching game while still giving players the option. Hence we implemented a battle-mode, a real-time 1v1 match against a real player where both board states are synchronized and the first person to match all tiles wins. /~Java Backend & Board State~/For real-time syncing of user boards we made Java backend services, where match state (board seed, player scores, and game status) is managed in a central shared document and streamed to both players via API polling/WebSocket updates. Because of this, game state was consistent across clients through shared match sessions and board generation./~Match End~/When a player finishes (only one player needs to match all tiles) the final score is written to the backend, and the winner is computed server-side. Both players are redirected to the results screen and player stats are updated accordingly (XP and battle stats)",
        "The leaderboard is implemented as a cloud-synced ranking system using Java and Spring Boot. User data is retrieved from two of our RESTful APIs, one for solo mode data and one for battle mode data, both being synced up by user ID. Updates are automatically streamed in using polled DB queries and indexing. Queries are polled such that performance isn't hindered while users are sitting on the leaderboard screen, to verify this we did some performance profiling using Flutter's integrated tools. "]
    },
